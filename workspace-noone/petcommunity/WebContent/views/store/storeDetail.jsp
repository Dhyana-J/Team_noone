...
<body>
    ...
    <%if(loginUser != null) {%>
    	<a href="<%=context %>/buyproduct.st?pno=<%=p.getpNo() %>" class="btn btn-secondary btn-sm" align="center">상품구매testing</a>
    <%} %>
    	<%if(loginUser != null && loginUser.getManager().equals("Y")) {%>
    		
	    	<div align="right">
	    		<a href="<%=context %>/storeUpdate.st?pno=<%=p.getpNo() %>" class="btn btn-secondary btn-sm" align="center">상품수정</a>
	    		<a href="<%=context %>/storeDelete.st?pno=<%=p.getpNo() %>" class="btn btn-danger btn-sm" align="center">상품삭제</a>
	    	</div>
	    <%} %>
    <div id= "area1">
            ...
            <table>
                ...
                <%}else{ %>
	                <%for(Review r :reviewList){ %>  
	                 <tbody>
	                 <tr>
	                    <td class="not_btn"><%=r.getReviewNO()%></td>
	                    <td class="not_btn"><%=r.getUserName() %></td>
	                    <td class="not_btn"><%=r.getReviewRating() %></td>
	                    <td class="not_btn"><%=r.getReviewTitle() %></td>
	                    <td>
	                        <div id="enroll_date1">
	                            <div style="width:80%;">
	                                <%=r.getReviewDate() %>
		                            ...
		                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#myModal" data-num="<%=r.getReviewNO()%>" data-no="<%=r.getUserNo() %>" >신고하기</button>
...
<script>
    $(function(){
        ...
        $(".not_btn").click(function(){
			...
			var rno = $("#comment_form tbody>tr").children().eq(0).text();
			console.log(rno);
		...
		$("#call").click(function(){
		    	  ...
		    	  function call(){
		    		  $.ajax({
		    			  url:"call.re?",
		    			  type:"POST",
		    			  data:{call_reason:$("#call_reason").val(),
		    				  call_reason_detail:$("#call_reason_detail").val(),
		    				  review_no:num,
		    				  userNo:no},
