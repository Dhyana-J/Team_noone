       ...
       <h1 align="center">재고관리</h1>
        ...
        <div class="paging-area" align="center">
			<%if(Pinfo.getCurrentPage() != 1){ %>
            	<a href="<%=context%>/stock.st?Page=<%=Pinfo.getCurrentPage() - 1%>&Psort=<%=Psort %>">&lt; 이전 </a>
            <%} %>
			<%for(int i = Pinfo.getStartPage(); i < Pinfo.getEndPage() + 1; i++) {%>
            	<a href="<%=context%>/stock.st?Page=<%=i%>&Psort=<%=Psort %>"><%=i %></a>
        	<%} %>
        	<%if(Pinfo.getMaxPage() > 0 && Pinfo.getMaxPage() != Pinfo.getCurrentPage()){ %>
            	<a href="<%=context%>/stock.st?Page=<%=Pinfo.getCurrentPage() + 1%>&Psort=<%=Psort %>">다음 &gt;</a>
            <%} %>
    ...
    <script>
        	$(function(){
        		$(".thumbnail").click(function(){
        			location.href = "<%=context%>/detail.st?pno="+$(this).children().eq(0).val()+"&rp=1&qp=1&pip=1";
        		});
