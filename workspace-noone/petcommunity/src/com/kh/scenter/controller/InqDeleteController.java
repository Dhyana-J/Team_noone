...
public class InqDeleteController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = new InqService().deleteInq(ino);
		
		if(result >0) {
			request.getSession().setAttribute("alertMsg", "성공적으로 제거되었습니다.");
			
			response.sendRedirect(request.getContextPath() + "/list.inq?ino="+ino);						
		}else {
			request.getSession().setAttribute("errorMsg", "제거에 실패했습니다.");
