...
public class ProductService {
	...
	public int StoreUpdate(Product p, int pno) {
		Connection conn = getConnection();
		int result = new ProductDao().StoreUpdate(conn, p, pno);
	...
	public int StoreDelete(int pno) {
		Connection conn = getConnection();

		int result = new ProductDao().StoreDelete(conn, pno);
	...
	public int Buy(int number, int pno) {
		Connection conn = getConnection();
		Product p = BselectP(pno);
		int sales = p.getpSales();
		int tsales = sales + number;
		int result2 = new ProductDao().directSale(conn, tsales, pno);
		int result = new ProductDao().Buy(conn, number, p.getpStock(), pno);
		
		close(conn);
		return result * result2;
	}
	
	public Product BselectP(int pno) {
		Connection conn = getConnection();
		Product p = new ProductDao().BselectP(conn, pno);
		close(conn);
		return p;
	}
	public int insertOder(int userNo) {
		Connection conn = getConnection();
		int result = new ProductDao().insertOder(conn,userNo);
		if(result>0) {
			commit(conn);
		}else {
			rollback(conn);
